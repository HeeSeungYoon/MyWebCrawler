...
class MyApp(QWidget):
    ...
    def crawling(self):
        ...
        self.driver.get('https://images.google.co.kr/imghp?hl=ko&gws_rd=ssl')
    ...
    # def saving(self):    
        ...
        for image in self.images:
            try:
                image.click()
                time.sleep(1)
                imageUrl = self.driver.find_element(By.XPATH,'//*[@id="Sva75c"]/div[2]/div[2]/div[2]/div[2]/c-wiz/div/div/div/div[3]/div[1]/a/img[1]').get_attribute('src')
            ...
            cnt += 1
            self.worker.run()
            if cnt >= self.progressBar.maximum():
                self.worker.stop()
